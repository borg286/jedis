...
public class JedisWithCompleteCredentialsTest extends JedisCommandTestBase {
  ...
  public void timeoutConnectionWithURI() throws Exception {
    Jedis jedis = new Jedis(new URI("redis://default:foobared@localhost:6379/2"), 15000);
    String timeout = jedis.configGet("timeout").get(1);
    jedis.configSet("timeout", "1");
    Thread.sleep(2000);
    try {
    ...
    // reset config
    jedis = new Jedis(new URI("redis://default:foobared@localhost:6379/2"));
    jedis.configSet("timeout", timeout);
    jedis.close();
  }
  ...
  public void shouldThrowInvalidURIExceptionForInvalidURI() throws URISyntaxException {
    Jedis j = new Jedis(new URI("localhost:6379"));
    j.ping();
  }
  ...
  public void startWithUrlString() {
    ...
    j.select(2);
    j.set("foo", "bar");
    Jedis jedis = new Jedis("redis://default:foobared@localhost:6379/2");
  ...
  public void startWithUrl() throws URISyntaxException {
    ...
    j.select(2);
    j.set("foo", "bar");
    Jedis jedis = new Jedis(new URI("redis://default:foobared@localhost:6379/2"));
    assertEquals("PONG", jedis.ping());
    assertEquals("bar", jedis.get("foo"));
  }
